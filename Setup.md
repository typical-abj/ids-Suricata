...
Before installing the suricata, we must install the Prerequisites for it. 

Prerequisites are the conditions that must be ready and must be true before suricata can finction.

   #sudo apt -y install libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 jq
 
 Now we add the Respositories.
...
Now lets enable and Check the suricata service.

   #sudo systemctl enable suricata.service      
   #sudo systemctl status suricata.service

We can see its active but exited. Now lets stop the suricata and continue the configuration.
...
Remove the hash from EXTERNAL_NET  and make it "any" so that every other devices will be consider as an outsider and monitor its incoming packets.
